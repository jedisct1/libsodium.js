...
for (const macroFile of macrosFiles) {
  ...
  const macroCode = fs.readFileSync(
    path.join(__dirname, "macros", macroFile),
    { encoding: "utf8" }
  );
...
function buildSymbol(symbolDescription: Symbol): void {
  ...
  if (symbolDescription.type === "function") {
    const targetName = "libsodium._" + symbolDescription.name;
    ...
    for (const currentParameter of inputs) {
      ...
      const substitutions: Substitution[] = [{ from: "{var_name}", to: currentParameter.name }];
    ...
    for (const currentOutput of outputs) {
      ...
      const substitutions: Substitution[] = [{ from: "{var_name}", to: currentOutput.name }];

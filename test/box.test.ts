...
test("crypto_box_detached returns separate ciphertext and MAC", () => {
    ...
    const result = sodium.crypto_box_detached(message, nonce, bobPublic, aliceSecret);
...
test("crypto_box_open_detached round-trip", () => {
    ...
    const { ciphertext, mac } = sodium.crypto_box_detached(message, nonce, bobPublic, aliceSecret);
    
    // Bob decrypts from Alice
    const decrypted = sodium.crypto_box_open_detached(ciphertext, mac, nonce, alicePublic, bobSecret);
...
test("crypto_box_open_detached with tampered MAC fails", () => {
    ...
    expect(() => {
        sodium.crypto_box_open_detached(ciphertext, tamperedMac, nonce, alicePublic, bobSecret);
    }).toThrow();
});
...
test("crypto_box_open_detached with tampered ciphertext fails", () => {
    ...
    expect(() => {
        sodium.crypto_box_open_detached(tamperedCiphertext, mac, nonce, alicePublic, bobSecret);
    }).toThrow();
});
...
test("crypto_box_open_detached with wrong key fails", () => {
    ...
    expect(() => {
        sodium.crypto_box_open_detached(ciphertext, mac, nonce, alicePublic, eveSecret);
    }).toThrow();
});

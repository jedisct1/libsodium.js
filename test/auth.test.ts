...
describe("crypto_auth_hmacsha256", () => {
	...
	test("verification fails with wrong MAC", () => {
		...
		wrongMac[0] ^= 0xff;
		expect(sodium.crypto_auth_hmacsha256_verify(wrongMac, message, key)).toBe(
			false,
		);
	});
	...
	test("verification fails with wrong key", () => {
		...
		expect(sodium.crypto_auth_hmacsha256_verify(mac, message, key2)).toBe(
			false,
		);
	});
...
describe("crypto_auth_hmacsha512", () => {
	...
	test("verification fails with wrong MAC", () => {
		...
		const wrongMac = new Uint8Array(mac);
		wrongMac[0] ^= 0xff;
		expect(sodium.crypto_auth_hmacsha512_verify(wrongMac, message, key)).toBe(
			false,
		);
	});
	...
	test("verification fails with wrong key", () => {
		...
		expect(sodium.crypto_auth_hmacsha512_verify(mac, message, key2)).toBe(
			false,
		);
	});
...
describe("crypto_auth_hmacsha512256", () => {
	...
	test("verification succeeds with correct MAC", () => {
		...
		const message = sodium.from_string("Message to verify");
		const mac = sodium.crypto_auth_hmacsha512256(message, key);
		expect(sodium.crypto_auth_hmacsha512256_verify(mac, message, key)).toBe(
			true,
		);
	});
	...
	test("verification fails with wrong MAC", () => {
		...
		wrongMac[0] ^= 0xff;
		expect(
			sodium.crypto_auth_hmacsha512256_verify(wrongMac, message, key),
		).toBe(false);
	});
	...
	test("verification fails with wrong key", () => {
		...
		expect(sodium.crypto_auth_hmacsha512256_verify(mac, message, key2)).toBe(
			false,
		);
	});

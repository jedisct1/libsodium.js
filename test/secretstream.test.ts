...
test("crypto_secretstream_xchacha20poly1305_keygen generates correct size key", () => {
    ...
    expect(key.length).toBe(sodium.crypto_secretstream_xchacha20poly1305_KEYBYTES);
});
...
test("crypto_secretstream_xchacha20poly1305_init_push returns state and header", () => {
    ...
    expect(result.header.length).toBe(sodium.crypto_secretstream_xchacha20poly1305_HEADERBYTES);
});
...
test("crypto_secretstream_xchacha20poly1305_push encrypts message", () => {
    ...
    expect(ciphertext.length).toBe(message.length + sodium.crypto_secretstream_xchacha20poly1305_ABYTES);
});
...
test("crypto_secretstream_xchacha20poly1305_push multiple chunks", () => {
    ...
    for (let i = 0; i < messages.length - 1; i++) {
        ciphertexts.push(sodium.crypto_secretstream_xchacha20poly1305_push(
            state,
            messages[i],
            null,
            sodium.crypto_secretstream_xchacha20poly1305_TAG_MESSAGE
    ...
    ciphertexts.forEach((ct, i) => {
        expect(ct.length).toBe(messages[i].length + sodium.crypto_secretstream_xchacha20poly1305_ABYTES);
    });
});
...
test("crypto_secretstream_xchacha20poly1305_pull decrypts message", () => {
    ...
    const pullState = sodium.crypto_secretstream_xchacha20poly1305_init_pull(header, key);
    const result = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext, null);
...
test("crypto_secretstream_xchacha20poly1305_pull with additional data", () => {
    ...
    const pullState = sodium.crypto_secretstream_xchacha20poly1305_init_pull(header, key);
    const result = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext, ad);
...
test("crypto_secretstream_xchacha20poly1305_pull with wrong additional data fails", () => {
    ...
    const result = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext, wrongAd);
...
test("crypto_secretstream_xchacha20poly1305 full round-trip single message", () => {
    ...
    const originalMessage = sodium.from_string("This is a test message for encryption.");
    const ciphertext = sodium.crypto_secretstream_xchacha20poly1305_push(
        pushState,
        originalMessage,
        null,
    ...
    const pullState = sodium.crypto_secretstream_xchacha20poly1305_init_pull(header, key);
    const { message, tag } = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext, null);
...
test("crypto_secretstream_xchacha20poly1305 full round-trip multiple messages", () => {
    ...
    // Verify tags
    for (let i = 0; i < messages.length - 1; i++) {
        expect(tags[i]).toBe(sodium.crypto_secretstream_xchacha20poly1305_TAG_MESSAGE);
    }
    expect(tags[messages.length - 1]).toBe(sodium.crypto_secretstream_xchacha20poly1305_TAG_FINAL);
});
...
test("crypto_secretstream_xchacha20poly1305_rekey during encryption", () => {
    ...
    const result1 = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext1, null);
    ...
    const result2 = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext2, null);
...
test("crypto_secretstream_xchacha20poly1305_rekey with TAG_REKEY", () => {
    ...
    const result1 = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext1, null);
    expect(result1.message).toEqual(messages[0]);
    expect(result1.tag).toBe(sodium.crypto_secretstream_xchacha20poly1305_TAG_REKEY);
    
    // Pull should automatically rekey based on TAG_REKEY
    const result2 = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext2, null);
    expect(result2.message).toEqual(messages[1]);
    expect(result2.tag).toBe(sodium.crypto_secretstream_xchacha20poly1305_TAG_FINAL);
});
...
test("crypto_secretstream_xchacha20poly1305 decryption with wrong key fails", () => {
    ...
    const pullState = sodium.crypto_secretstream_xchacha20poly1305_init_pull(header, wrongKey);
...
test("crypto_secretstream_xchacha20poly1305 decryption with corrupted ciphertext fails", () => {
    ...
    const result = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, corruptedCiphertext, null);
    expect(result).toBe(false);
});
...
test("crypto_secretstream_xchacha20poly1305 decryption with corrupted header fails", () => {
    ...
    const pullState = sodium.crypto_secretstream_xchacha20poly1305_init_pull(corruptedHeader, key);
    // But pull should fail to decrypt - returns false
    const result = sodium.crypto_secretstream_xchacha20poly1305_pull(pullState, ciphertext, null);
    expect(result).toBe(false);
});
...
test("crypto_secretstream_xchacha20poly1305_TAG_PUSH", () => {
    ...
    expect(result.message).toEqual(message);
    expect(result.tag).toBe(sodium.crypto_secretstream_xchacha20poly1305_TAG_PUSH);
});
...
test("crypto_secretstream_xchacha20poly1305 constants are defined", () => {
    ...
    expect(sodium.crypto_secretstream_xchacha20poly1305_HEADERBYTES).toBeDefined();
    expect(sodium.crypto_secretstream_xchacha20poly1305_ABYTES).toBeDefined();
    expect(sodium.crypto_secretstream_xchacha20poly1305_TAG_MESSAGE).toBeDefined();
...
test("crypto_secretstream_xchacha20poly1305 streaming large file simulation", () => {
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        if (i < chunks.length - 1) {
            expect(tag).toBe(sodium.crypto_secretstream_xchacha20poly1305_TAG_MESSAGE);
        } else {
            expect(tag).toBe(sodium.crypto_secretstream_xchacha20poly1305_TAG_FINAL);
        }
    }

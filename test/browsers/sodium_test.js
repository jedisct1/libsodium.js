var sodium_test = function () {
	...
	exports.scrypt = function () {
		if (typeof sodium.crypto_pwhash_scryptsalsa208sha256 != "function") throw new Error("scrypt cannot be found in the js-built library");
		var vectors = [{
			pass: "password",
			salt: "NaCl",
			keyLength: 64,
	...
	exports.secretbox_easy = function () {
		var vectors = [{
	...
	exports.hash = function () {
		...
		var i;
		if (sodium.crypto_hash)
			for (i = 0; i < sha512Vectors.length; i++) sha512_test(sha512Vectors[i]);
		if (sodium.crypto_hash_sha256)
			for (i = 0; i < sha256Vectors.length; i++) sha256_test(sha256Vectors[i]);
	...
	exports.hmac = function () {
		...
		var i;
		if (sodium.crypto_auth)
			for (i = 0; i < hmacSHA512256Vectors.length; i++) testHmacSHA512256Vector(hmacSHA512256Vectors[i]);
		if (sodium.crypto_auth_hmacsha512)
			for (i = 0; i < hmacSHA512Vectors.length; i++) testHmacSHA512Vector(hmacSHA512Vectors[i]);
		if (sodium.crypto_auth_hmacsha256)
			for (i = 0; i < hmacSHA256Vectors.length; i++) testHmacSHA256Vector(hmacSHA256Vectors[i]);

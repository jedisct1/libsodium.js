...
describe('libsodium compatibility', () => {
    ...
    it('crypto_generichash', async () => {
        ...
        let mac1 = sodium.crypto_generichash(32, plaintext, key);
        expect(Buffer.from(mac1).toString('hex'))
            .to.be.equals('e229536f8c0d462126f040126392b46151200531f7bd12061a2237833a0ccdba');
        ...
        let mac2 = sodium.crypto_generichash_final(state, 32);
        let match = sodium.memcmp(mac1, mac2);
        expect(match).to.be.equal(true);

        let mac3 = sodium.crypto_generichash(32, plaintext + "extra", key);
        match = sodium.memcmp(mac1, mac3);
    ...
    it('crypto_auth_hmacsha512256', async () => {
        ...
        let key = Buffer.from('4777a57dadf099111c8c21954b0b470b1990f34623990d32bf0340795ff858d8', 'hex');
        let plaintext = "This is just - something to authenticate";
        let mac1 = sodium.crypto_auth_hmacsha512256(plaintext, key);
        expect(Buffer.from(mac1).toString('hex'))
            .to.be.equals('0e2e248662c176f59753ad6c3a6a09826c3e92a449179e07e0ac712c10c56d33');
        ...
        let mac2 = sodium.crypto_auth_hmacsha512256_final(state);
        let match = sodium.crypto_auth_hmacsha512256_verify(mac2, plaintext, key);
        expect(match).to.be.equal(true);

        let mac3 = sodium.crypto_auth_hmacsha512256(plaintext + "extra", key);
        match = sodium.crypto_auth_hmacsha512256_verify(mac3, plaintext, key);
        expect(match).to.be.equal(false);
    });
});
